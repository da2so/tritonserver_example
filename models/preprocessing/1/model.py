...
def transform_image(image):
    ...
    """
    IM_SIZE=224
    transform = A.Compose([A.Resize(IM_SIZE, IM_SIZE), A.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225))])
...
class TritonPythonModel: 
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ## get request 
            raw_images = pb_utils.get_input_tensor_by_name(request, "RAW_IMAGE").as_numpy()

            ## make response
            input_image = gen_input_image(raw_images)
            input_image_tensor = pb_utils.Tensor("INPUT_IMAGE", input_image.astype(np.float32))
            response = pb_utils.InferenceResponse(output_tensors=[input_image_tensor])
